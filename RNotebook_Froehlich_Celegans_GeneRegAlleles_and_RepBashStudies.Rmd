---
title: "Published C. elegans Gene-Regulatory Alleles and Reporter Bashing Studies"
output:
  html_notebook:
    theme: readable
    code_folding: hide
...
# this supresses warnings and messages throughout the whole html report
```
...
### 0,1,2 Setup, Libraries, Datasets

Designate the working directory first
...
+ download datasets (~1 GB)
<br>incl. manually annotated Extended Data.
+ load libraries
+ load datasets
...
##+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## checking if data sets exists, if not downloads them, ~760 MB
##+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
This step cleans up transcriptome annotation to remove overlaps conservatively (e.g., 3′UTR sequence that overlaps with coding sequence). 

```{r}

##################################################/
## 3 Transcriptome annotation cleanup ####
...
The script then exports a .pdf with browser shots focussing on five random and two chosen genes to show examples of affected transcriptome annotation.  
```{r, results='hide'}
...
Here an example of 3'/5'UTRs overlapping with coding sequence (light grey), and the cleaned up annotation (dark grey):
```{r, out.width="75%", fig.cap="Figure: Genome browser shot showing annotation before (light grey) and after (dark grey) cleanup."}

g_coord <- genes_to_plot.dt[genes_to_plot.dt$gene_id=="WBGene00302984",]
...
### 4 Annotation of datasets

#### Automated annotation

Here the alleles are categorized according to their overlap with genome feature (e.g., coding, synonymous, 5′-/3′UTR, intergenic).

<br>
This information is then exported as a spreadsheet .xlsx file for manual annotation in: "output/table_for_manual_annotation.xlsx".
Additionally the ranges of alleles are exported by their overlap into different .bed files, which can be inspected using the UCSC genome browser. 
...
#### Manual curation using literature

Together with literature searches the .xlsx file was then manually populated with extra information using a spreadsheet program. 
Here we removed alleles that were not non-coding and added alleles that were not registered at Wormbase. Please see Methods of the accompanying publication for details. 
After the manual annotation, the annotated file was saved as "supplemental_data/Extended_Data_Tables_S1_S2_S3.xlsx". 
<br>
<br>
We provide this annotated table in our Extended Data. The script will automatically download it. 
...
```
The script loads the manually curated spreadsheet (= Extended Data from the publication). It then prepares plots, which it also exports into a document under: "output/plot_classical_alleles.pdf". 
<br>
<br>

#### Overview alleles & non-coding alleles
We first looked at the classical alleles (which mostly came from forward genetics screens). 
At this point we still excluded any CRISPR-engineered alleles.
<br>
<br>
Which underlying molecular consequences contributed to the alleles?
```{r}
## plots
#-------/
...
```{r, out.width="75%"}
ggplot(classical_alleles.dt[which(is.na(classical_alleles.dt$engineered)),] %>% dplyr::count(consequence) %>% mutate(pct=n/sum(n)), aes(x="", n, fill=consequence)) +
  geom_bar(stat="identity", alpha=0.8, colour="black", linewidth=0.35, width=0.4) +
  geom_text_repel(box.padding=0.07, size=3, xlim=c(1.25,5), max.overlaps=Inf, 
                  position=position_stacknudge(vjust = 0.5, x = 0.22), # seems to not work after latest updates
                  aes(y=pct*sum(n), label=paste0(sprintf("%1.1f", pct*100),"%"))) +
...
ggplot(classical_alleles_post_ManAnnot.dt[which(is.na(classical_alleles_post_ManAnnot.dt$engineered_postManAnnot)),] %>% dplyr::count(consequence_postManAnnot) %>% mutate(pct=n/sum(n)), aes(x="", n, fill=consequence_postManAnnot)) +
  geom_bar(stat="identity", alpha=0.8, colour="black", linewidth=0.35, width=0.4) +
  geom_text_repel(box.padding=0.07, size=3, xlim=c(1.25,5), max.overlaps=Inf, 
                  position=position_stacknudge(vjust = 0.5, x = 0.22), # seems to not work after latest updates
                  aes(y=pct*sum(n), label=paste0(sprintf("%1.2f", pct*100),"%"))) +
...
<br>
Which mutation types contributed most to these alleles?
```{r, out.width="75%", fig.cap="."}
ggplot(classical_alleles_post_ManAnnot.dt[which(is.na(classical_alleles_post_ManAnnot.dt$engineered_postManAnnot)),] %>% dplyr::count(type) %>% mutate(pct=n/sum(n)), aes(x="", n, fill=forcats::fct_reorder(type, pct, .desc=TRUE))) +
  geom_bar(stat="identity", alpha=0.5, colour="black", linewidth=0.35, width=0.2) +
  geom_text_repel(box.padding=0.07, size=3, xlim=c(1.20,5), max.overlaps=Inf, 
                  position=position_stacknudge(vjust = 0.5, x = 0.13), # seems to not work after latest updates
                  aes(y=pct*sum(n), label=paste0(sprintf("%1.1f", pct*100),"%"))) +
...
<br>
Which fraction of alleles are non-coding (not affecting coding sequence)?
```{r, out.width="75%", fig.cap="."}
ggplot(classical_alleles_post_ManAnnot.dt[which(is.na(classical_alleles_post_ManAnnot.dt$engineered_postManAnnot)),] %>% dplyr::count(con_sum1) %>% mutate(pct=n/sum(n)), aes(x="", n, fill=con_sum1)) +
  geom_bar(stat="identity", alpha=0.5, colour="black", linewidth=0.35, width=0.2) +
  geom_text_repel(box.padding=0.07, size=3, xlim=c(1.20,5), max.overlaps=Inf, 
                  position=position_stacknudge(vjust = 0.5, x = 0.13), # seems to not work after latest updates
                  aes(y=pct*sum(n), label=paste0(sprintf("%1.1f", pct*100),"%"))) +
...
<br>
Which gene regulatory regions are affected by these non-coding alleles ?
```{r, out.width="75%", fig.cap="."}
ggplot(classical_alleles_post_ManAnnot.dt[which(classical_alleles_post_ManAnnot.dt$con_sum1 == "non-coding" & is.na(classical_alleles_post_ManAnnot.dt$engineered_postManAnnot)),] 
             %>% dplyr::count(consequence_postManAnnot) %>% mutate(pct=n/sum(n)), aes(x="", n, fill=consequence_postManAnnot)) +
  geom_bar(stat="identity", alpha=0.8, colour="black", linewidth=0.35, width=0.2) +
  geom_text_repel(box.padding=0.07, size=3, xlim=c(1.13,5), max.overlaps=Inf, 
                  position=position_stacknudge(vjust = 0.5, x = 0.13), # seems to not work after latest updates
...
#### Overview endogenous mutation studies
Which regulatory regions did each of the endogenous mutation studies (= forward genetics or CRISPR-engineered) focus on?
We now included CRISPR studies. 
```{r, results='hide'}
...
```{r, out.width="75%", fig.cap="."}
ggplot(unique(classical_alleles_post_ManAnnot.dt[,c("ManAnnot_ref_regions","con_sum1","ManAnnot_reference", "ManAnnot_ref_year")][which(classical_alleles_post_ManAnnot.dt$con_sum1 == "non-coding" & classical_alleles_post_ManAnnot.dt$consequence_postManAnnot != c("synonymous_variant")),]) 
             %>% dplyr::count(ManAnnot_ref_regions) %>% mutate(pct=n/sum(n)), aes(x="", n, fill=ManAnnot_ref_regions)) +
  geom_bar(stat="identity", alpha=0.8, colour="black", linewidth=0.35, width=0.2) +
  geom_text_repel(box.padding=0.07, size=3, xlim=c(1.13,5), max.overlaps=Inf, 
                  position=position_stacknudge(vjust = 0.5, x = 0.13), # seems to not work after latest updates
...
#### Overview reporter bashing studies
Which regulatory regions did each of the manually collected reporter bashing studies focus on?
```{r}
...
```{r, out.width="75%", fig.cap="."}
ggplot(reporter_annot %>% dplyr::count(overlap_feature_R) %>% mutate(pct=n/sum(n)), aes(x="", n, fill=overlap_feature_R)) +
  geom_bar(stat="identity", alpha=0.8, colour="black", linewidth=0.35, width=0.2) +
  geom_text_repel(box.padding=0.07, size=3, xlim=c(1.17,5), max.overlaps=Inf, 
                  position=position_stacknudge(vjust = 0.5, x = 0.11), # seems to not work after latest updates
                  aes(y=pct*sum(n), label=paste0(pct*sum(n), "/", sum(n), "  (", sprintf("%1.1f", pct*100),"%)"))) +
...
#### Tested mutations per study
How many mutations (variants) did each study test?
```{r, results='hide'}
...
p2 <- ggplot(variants_per_study.dt, aes(y=variants_per_study, x=factor(type), group=type)) +
  ...
  geom_boxplot(width=0.07, color="black", linewidth=0.4, outlier.shape=NA, alpha=0.5) +
...
#   geom_boxplot(width=0.07, color="black", linewidth=0.7, outlier.shape=NA) +
...
```{r, out.width="75%", fig.cap="."}
ggplot(variants_per_study.dt, aes(y=variants_per_study, x=factor(type), group=type)) +
  geom_violin(trim=TRUE, color=NA, fill="#d1d1d1") + # bw=1
  geom_boxplot(width=0.07, color="black", linewidth=0.4, outlier.shape=NA, alpha=0.5) +
  geom_dotplot(binaxis='y', stackdir='center', dotsize=0.3, stackratio=2, color=NA, fill="black", alpha=0.25) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x, n = 4),
...
#### Regulatory activity of regions
Which gene regulatory activity did the studies find for the different categories of regulatory region (transcriptional cis-reg, 5'UTR, 3'UTR) ?
```{r, include=FALSE}
...
p1 <- ggplot(sequence_function.dt, aes(x=type, y=pct, fill=ManAnnotIn53_mutation_expression_consequence, group=N)) +
  geom_bar(stat="identity", position = "stack", alpha=0.5, colour="black", linewidth=0.4, width=1) +
...
```{r, out.width="75%", fig.cap="."}
ggplot(sequence_function.dt, aes(x=type, y=pct, fill=ManAnnotIn53_mutation_expression_consequence, group=N)) +
  geom_bar(stat="identity", position = "stack", alpha=0.5, colour="black", linewidth=0.4, width=1) +
  facet_grid( ~ overlap_feature_R_postManAnnot, scales = "free_x", space = "free") + 
  # facet_grid( ~ overlap_feature_R_postManAnnot) + # this keeps an empty space were no values are present (5'UTR, reporter)
  scale_y_continuous(expand = expansion(mult = c(0, 0.05))) +
...
#### Distance of transcriptional regulatory elements
At which distances to the target genes TSS are transcriptional cis-regulatory sequences usually found?
```{r}
## panel: distance
DT <- reporter_annot[
...
```{r, out.width="75%", fig.cap="."}
ggplot(distance.dt, aes(y=as.numeric(distance), x=factor(type), group=type)) +
  geom_violinhalf(trim=TRUE, color=NA, fill="#d1d1d1", position = position_nudge(x=0.1,y=0)) + # bw=1
  geom_boxplot(width=0.15, color="black", size=0.4, outlier.shape=NA, alpha = 0.5, position = position_nudge(x=-0.05,y=0), fill="grey") +
  geom_dotplot(binaxis='y', stackdir='up', dotsize=0.4, stackratio=1.7, color=NA, fill="black", alpha=0.2, position=position_nudge(x=0.1)) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x, n=7),
...
<br>
<br>
*"Published C. elegans Gene-Regulatory Alleles and Reporter Bashing Studies"*
<br>Jonathan J. Froehlich, Nikolaus Rajewsky
